...
function IssueChatPopup({ isOpen, onClose, issue }) {
  ...
  const fetchConversations = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/chat/issue/${issue.id}/conversations/`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        if (user?.id === issue.user && data.length > 0) {
          if (data.length === 1) {
  ...
  const fetchMessages = async (userId = null) => {
    try {
      const url = userId
        ? `http://localhost:8000/api/chat/issue/${issue.id}/messages/?user_id=${userId}`
        : `http://localhost:8000/api/chat/issue/${issue.id}/messages/`;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (user?.id === issue.user && selectedUserId) {
      ...
      <div className="bg-white rounded-lg w-full max-w-md h-96 flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex items-center space-x-2">
            {user?.id === issue.user && conversations.length > 1 && (
              <button
